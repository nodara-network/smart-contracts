...
describe.skip("nodara - deposit_funds", () => {
  ...
  it("Allows multiple deposits (adds up)", async () => {
    ...
    await program.methods.depositFunds(taskId, firstAmount).accountsPartial({
      creator: wallet.publicKey,
      taskAccount: taskPDA,
      rewardVault: vaultPDA,
    }).rpc();

    await program.methods.depositFunds(taskId, secondAmount).accountsPartial({
      creator: wallet.publicKey,
      taskAccount: taskPDA,
      rewardVault: vaultPDA,
    }).rpc();
  ...
  it("Fails on zero lamport deposit", async () => {
    ...
    try {
      await program.methods.depositFunds(taskId, zeroAmount).accountsPartial({
        creator: wallet.publicKey,
        taskAccount: taskPDA,
        rewardVault: vaultPDA,
      }).rpc();
